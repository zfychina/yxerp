<template>
  <van-sticky>
    <van-nav-bar title="产量与销量对比" left-arrow :right-text="currentDate" :border='false' fixed @click-left="onClickLeft"  @click-right="onClickright">
    </van-nav-bar>
  ...
  <van-row justify="center">
    <van-col span="23">
      ...
      <!--            // 对比折线图-->
                <profile-multiple v-if="flag && state.data"
                  :data="state.data?.[state.tabtitle[active]][0]"
                  :active='active'
...
import {getYearList, getPRcontrastReportsku} from "network/statement";
import ProfileMultiple from "components/chart/ProfileMultiple";

export default {
  name: "casaStatement",
  components: {ProfileMultiple},
  setup(){
    // 异步传值到子组件设定
  ...
  const state = reactive({
    ...
    onMounted(() => {
      ...
      setTimeout(()=>{multiple1.value=true},1000)

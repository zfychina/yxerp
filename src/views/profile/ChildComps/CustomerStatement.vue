<template>
  <van-sticky>
    <van-nav-bar title="客户统计" left-arrow :right-text="currentDate" :border='false' fixed @click-left="onClickLeft"  @click-right="onClickright">
    </van-nav-bar>
...
<!--                产品分类详情-->
                  <category-goods v-if="flag_category" :data="sortBykey(state.categorydata?.[state.tabtitle[active]], 'quantity')" :active='active' :year="currentDate" details= 'customer'></category-goods>
...
import {getYearList, getXScustomerReportsku, getXScustomerCateReportsku} from "network/statement";
...
export default {
  ...
  const state = reactive({
    ...
    const getreport = (year) => {
      ...
      getXScustomerReportsku(year).then(res=>{
        state.data = res[0]
        flag.value = true
    ...
    const getreport_cat = (year) => {
      getXScustomerCateReportsku(year).then(res=>{

<template>
  <van-sticky>
    <van-nav-bar title="销量统计" left-arrow :border='false' fixed @click-left="onClickLeft">
    </van-nav-bar>
  </van-sticky>
          ...
          <van-tab v-for="(item, index) in state.tabtitle" :title="item" :key="index">
            ...
            <van-pull-refresh v-model="state.refreshing" @refresh="onRefresh" success-text="刷新成功" style="min-height: 100vh;">
              ...
              >
                ...
                <!--                详情-->
                <category-goods  v-if="category_flag && state.categorydata?.[state.tabtitle[active]]" :data="sortBykey(state.categorydata?.[state.tabtitle[active]], 'quantity')"  :active='active' details='yearsagoods'></category-goods>
...
import {getYearsagoodsReport} from "network/statement";
...
export default {
  ...
  setup(){
    ...
    const getyeardata=() =>{
      getYearsagoodsReport('year').then(res=>{
        console.log(res);
    ...
    const getcategorydata=() =>{
      getYearsagoodsReport('category').then(res=>{
        console.log(res);

          ...
          <van-tab v-for="(item, index) in state.tabtitle" :title="item" :key="index">
            ...
            <van-pull-refresh v-model="state.refreshing" @refresh="onRefresh" success-text="刷新成功" style="min-height: 100vh;">
              ...
              >
                ...
                <!--                详情-->
                <category-goods  v-if="category_flag && state.categorydata?.[state.tabtitle[active]]" :data="sortBykey(state.categorydata?.[state.tabtitle[active]], 'quantity')" details='yearsales'></category-goods>

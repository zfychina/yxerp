<template>
  ...
  <van-form style="margin-top: 50px" @submit="onSubmit">
    ...
    <van-sticky :offset-top="45">
      ...
      <!--客户编号-->
      <!--ELEMENT-->
      <van-field scroll-to-error
                 autofocus
                 clickable
                 clearable
                 colon
                 readonly
                 v-model.trim="productline"
                 name="xs_order"
                 label="生产线"
                 placeholder="禁止编辑"
      />
...
<!--      </van-cell>-->
      ...
      <van-field v-show="!state.readonly"
                 ...
                 :rules="[{ validator: asyncValidator, message: '该生产订单已完成，请重新输入' }]"
      ...
      <van-field v-show="state.readonly"
                 ...
                 label="源生产订单"
                 placeholder="请输入生产订单编号"
                 :rules="[{ validator, message: '该生产订单已完成，请核实' }]"
...
export default {
  ...
  setup() {
    ...
    const validator = (val) => countOrderSCRK(val).then(res => {
      ...
      if(res.count >= 1) {
        ...
        Dialog.confirm({
          message: '是否打开已存在的订单？',
            ...
            .then(() => {
              // on confirm
              getOrderSCRKdetail(val).then(res=>{
                console.log(res);
                if (res.count===0){
                  //先清空前面的遗留数据
    ...
    // 源销售订单校验
    const asyncValidator = (val) => countOrderSCok(val).then(res => {
      console.log(res);
      if(res.count >= 1) {
        Dialog.confirm({
          message: '是否打开未完成的销售订单？',
            ...
            .then(() => {
              // on confirm
              getOrderSCokdetail(val).then(res=>{
                console.log(res);
                if (res.count===0){
                  //先清空前面的遗留数据
      ...
      if(order.value){
        Dialog.confirm({
          message: `删除订单，请点击"确认"`,
            ...
            .then(() => {
              // 删除订单
              deleteorderSCRK({'order':order.value}).then(res=>{
                Toast(res)
              })
            })
            .catch(() => {

<template>
  <van-sticky>
    <van-row>
      <van-col span="24">
        <van-nav-bar left-arrow title="生产入库" :border='false' @click-left="onClickLeft" @click-right="onClickRight">
          <template #right>
      ...
      <van-config-provider :theme-vars="themeVars">
        <van-form @submit="onSubmit">
          <van-cell-group inset>
            ...
            <field-cell title="订单编号" colon placeholder="   入库单号自动生成" name="orderhao" :autodata="state.orderhaodata" :data="orderhao" @inputvalue="receiveorderhaovalue" @onfocus="orderhaoonfocus" @onblur="orderhaoonblur"></field-cell>

            <field-cell title="生产线名" colon placeholder="   请输入生产线名" name="productline" :autodata="state.productlinedata" :data="productline" @inputvalue="receiveproductlinevalue" @onfocus="productlineonfocus" @onblur="productlineonblur"></field-cell>
...
import {productlinelist, productlinecount, orderSCRKlist, countOrderSCRK, getOrderSCRKdetail, orderSClist, countOrderSCok, getOrderSCokdetail, deleteorderSCRK, updateorderSCRK, createorderSCRK} from "network/unsettled";
...
export default {
  ...
  setup() {
    ...
    const orderhaoonblur = () => {
      ...
      // 校验生产订单是否存在
      if (orderhao.value) {
        countOrderSCRK(orderhao.value).then(res=>{
          console.log(res);
          if (res.count > 0) {
            Dialog.confirm({message: '是否打开已存在的订单？',}).then(() => {
              // on confirm
              getOrderSCRKdetail(orderhao.value).then(res=>{
                console.log(res);
                if (res.count===0){
                  //先清空前面的遗留数据
                  state.skulist = [{},{},{},{},{},{}]
                  ...
                  productline.value = res.results[0].sku.order.supplier.shortname
    ...
    const xsorderhaoonblur = () => {
      ...
      if(xsorderhao.value){
        countOrderSCok(xsorderhao.value).then(res=>{
          console.log(res);
          setTimeout(() => {}, 10);
          if(res.count >= 1){
            Dialog.confirm({message: '是否打开未完成的采购订单？',}).then(() => {
              // on confirm
              getOrderSCokdetail(xsorderhao.value).then(res=>{
                console.log(res);
                if (res.count===0){
                  //先清空前面的遗留数据
                  productline.value = ''
      ...
      if(orderhao.value){
        Dialog.confirm({
          message: `删除订单，请点击"确认"`,
            ...
            .then(() => {
              // 删除订单
              deleteorderSCRK({'order':orderhao.value}).then(res=>{
                Toast(res)
              })
            })
            .catch(() => {
    ...
    const onSubmit = () => {
      ...
      if (!data.xs_order){
        Toast({message:'请选择生产订单号', duration: 1000 })
        return
      }
      data.sku = []

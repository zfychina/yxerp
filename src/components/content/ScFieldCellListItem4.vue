<template>
  ...
  <div v-show="is_spread" >
    <sc-field-cell-list-item-end v-for="(goodsdata,index) in state.goodslist"  @inputvalue="receiveskulistvalue" :key="index" :serial="index" :data="goodsdata" @spreadbomone="spreadbomone"></sc-field-cell-list-item-end>
  </div>
...
import ScFieldCellListItemEnd from "./ScFieldCellListItemEnd";
import {getbomdetail} from 'network/bom'

export default {
  name: "ScFieldCellListItem",
  components: {ScFieldCellListItemEnd, AutoCell, AutoCellGood},
  props: {
    quantity: {
      type: Number,
    },

<template>
  ...
  <div v-show="is_spread" >
    <sc-field-cell-list-item1 v-for="(goodsdata,index) in state.goodslist"  @inputvalue="receiveskulistvalue" :key="index" :serial="index" :data="goodsdata" @spreadbomone="spreadbomone"></sc-field-cell-list-item1>
  </div>
...
import ScFieldCellListItem1 from "./ScFieldCellListItem1";
import {getbomdetail} from 'network/bom'

export default {
  name: "ScFieldCellListItem",
  components: {ScFieldCellListItem1, AutoCell, AutoCellGood},
  props: {
    quantity: {
      type: Number,
    },

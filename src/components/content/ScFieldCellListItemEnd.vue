<template>
  ...
  <van-swipe-cell :before-close="beforeClose">
    <van-row justify="left" :style="[{'background-color':(data.process===4?'#fffd0f':'#ffffff')},{fontSize:'12px'}]" align="center">
      <van-col span="2"  offset="1" style="font-size: 14px">end</van-col>
      <van-col span="11">
      ...
      <van-col span="6">
        ...
        <p>{{data.picking >0 ? '已提 ' + data.picking+'/'+data.stock : (data.coding ? (data.stock ? '库存 ' + data.stock : '') : '')}}</p>
      </span>
      </van-col>
    ...
    <template #right style="text-align: center">
      <van-button square type="primary" text="预料" style="height: 100%;"/>
    </template>
  </van-swipe-cell>

  <!--展开显示订单商品  1 -->
  <sc-field-cell-list-item2 v-show="is_spread" v-for="(goodsdata,index) in state.goodslist" :key="index" :serial="index" :data="goodsdata" @spreadbomone="spreadbomone"></sc-field-cell-list-item2>
...
import ScFieldCellListItem2 from "./ScFieldCellListItem2";

export default {
  name: "ScFieldCellListItem1",
  components: { AutoCell, AutoCellGood, ScFieldCellListItem2},
  props: {
    quantity: {
      type: Number,
    },

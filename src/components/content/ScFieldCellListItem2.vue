<template>
  ...
  <div v-show="is_spread" >
    <sc-field-cell-list-item3 v-for="(goodsdata,index) in state.goodslist"  @inputvalue="receiveskulistvalue" :key="index" :serial="index" :data="goodsdata" @spreadbomone="spreadbomone"></sc-field-cell-list-item3>
  </div>
...
import ScFieldCellListItem3 from "./ScFieldCellListItem3";
import {getbomdetail} from 'network/bom'

export default {
  name: "ScFieldCellListItem2",
  components: {ScFieldCellListItem3, AutoCell, AutoCellGood},
  props: {
    quantity: {
      type: Number,
    },

  ...
  setup(){
    ...
    onMounted(()=>{
      ...
      getMonthfinishinfo().then(res=> {
        state.data[0].actual = (res.count_total - res.count_unfinish) / res.count_total * 100
        console.log(state);
      }),
            ...
            appear: {
              ...
              animation: function animation(shape, animateCfg) {
                ...
                shape.animate().to(Util.mix({
                  ...
                  if (textEl) textEl.innerHTML = parseInt(frame * state.data[0].actual) + '%';
                });
              }
            }
          });

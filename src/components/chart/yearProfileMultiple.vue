<template>
  ...
  ">
    ...
    <van-row justify="center">
      <van-col span="4" v-for="(item_month, index) in item.month" :key="index" style="margin-top:15px;font-size: 14px; height: 40px;text-align: center;vertical-align: middle">
        <p>{{item_month.quantity}}</p>
        <p></p>
        <span style="font-weight: initial ;font-size: 10px;color: #d29403">{{item_month.month}}</span>
      </van-col>
    </van-row>
    </div>
  </div>
  ...
  <van-row>
    <van-col span="24">
      <canvas :id="myChart2" width="370" height="200"></canvas>
    </van-col>
  </van-row>
</template>
  ...
  setup(props){
    ...
    const changedata = ()=>{
      for (let i in props.data){
        chartdata.value =  chartdata.value.concat(props.data[i].month)
      }
    }
